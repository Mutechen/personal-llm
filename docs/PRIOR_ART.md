...
- **Draw architecture from**: Letta (memory tiers + MemFS direction), Karpathy LLM Wiki + claude-obsidian (cold knowledge + `hot.md` cache), Voyager (curriculum + skill library), smolagents (agent loop), Khoj (the closest analog — and a cautionary pivot story), Open WebUI (plugin pattern).
...
| **Letta (formerly MemGPT)** | Memory framework — core/recall/archival tiers, self-edit tools. We already adopted it for L4. | Letta-as-library; **their 2026 architecture shift** to client-side execution + git-backed memory + filesystem operations (their "MemFS") — confirms our Karpathy-Wiki-in-Obsidian direction. Their sleep-time compute paper and code. | Their lesson: "memory was the key to self-improving AI." Stateful agents with persistent memory outlast any single base model. Build memory as filesystem, not specialized tool. |
...
| **claude-obsidian / second-brain implementations** | Implementations of Karpathy's LLM Wiki pattern in Obsidian. | **`hot.md` cache pattern** — ~500 words of recent session context kept in a single file, refreshed at session boundaries. Reported ~71.5× token reduction vs. naive file feeding. Adopted for our Letta core memory's session tier. | The pattern works at real scale (Karpathy's own wiki: 100 articles, 400k words, agent-maintained). The maintenance burden is the bottleneck — LLMs solve it by not getting bored. |
...
- **The `hot.md` session cache** (claude-obsidian) — ~500 words of recent session context kept in a single file. Letta core memory loads it every turn. Claim: ~71.5× token reduction vs. naive file-feeding.
...
- **PostgresSaver checkpointing + user-scoped memory** (Letta production deployments) — but we use sqlite + filesystem for our single-user case; same idea, simpler infrastructure.
...
- **Sleep-time loop with explicit growth diffs** — sleep-time compute exists (Letta paper, April 2025). The "growth diff written to a file the user can audit in 60 seconds" framing is our addition, intended to make self-improvement legible and reversible.
...
- **Letta's continued evolution.** They rearchitected in 2026; if they release MemFS as a standalone library, evaluate replacing our Letta-as-library wiring with their MemFS directly.
