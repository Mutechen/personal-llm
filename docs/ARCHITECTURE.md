...
**Document library (RAG):** `personal-llm ingest <file>` parses `.txt/.md/.pdf/.epub` (`documents/parsers.py`; EPUB via stdlib zip+OPF, no lxml), chunks it (`documents/chunking.py`), embeds each chunk, and stores it (`documents` + `doc_chunks`). `books search` runs the same brute-force cosine as fact recall over those chunks. Idempotent by content hash. The nightly loop auto-ingests `raw/` (`ingest_directory`) and generates a wiki page per document (`documents/wiki.py` → `wiki/library/`: a local-model summary + key topics with a source citation, regenerated only when the source hash changes). Full document-level analysis / cross-linking is still later.
